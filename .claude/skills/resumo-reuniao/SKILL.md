...
Os templates 1 e 2 (Diagnóstico e Briefing de Cotação) têm campos estruturados fixos: preencha-os quando mencionados; quando não identificados na transcrição, mantenha o campo (não omita) e deixe-o em branco ou sinalize algo como "não identificado na transcrição". Essa é a única exceção à regra geral de "omitir o que não apareceu". Nos demais templates, se uma seção inteira não tiver conteúdo real na conversa, omita a seção.

**Regra transversal: dados de estudo de mercado aparecem em qualquer tipo de reunião** (confirmado pelo time em 22/07/2026). Os campos do template de Briefing de Cotação (para plano de saúde: operadoras, vidas, ticket médio, divisão CLT/PJ, aniversário da apólice, adesão, coparticipação, subsídio, elegibilidade, valor de reembolso, breakeven, sinistralidade, último reajuste, reclamações; para odontológico: vidas, operadora, data de renovação, adesão, subsídio, plano atual, ticket médio; para seguro de vida: operadora, link da apólice, vigência, capital segurado, coberturas) são relevantes para o processo comercial em qualquer etapa do funil, não só quando a reunião é classificada como Briefing de Cotação.

Se qualquer um desses dados aparecer numa reunião de outro tipo (o caso mais comum: o cliente já adianta essas informações numa Reunião de Diagnóstico, pedindo um estudo de mercado, mesmo sem ter havido um Briefing de Cotação formal ainda), registre-os no resumo mesmo assim. Não force a estrutura fixa do template de Briefing de Cotação nesses casos: inclua a informação de forma natural dentro do template do tipo real da reunião (ex.: como parte de "Dores e características do plano de saúde e demais benefícios" no template de Diagnóstico). **Isso não muda a classificação do tipo**: a reunião continua classificada pelo que ela realmente foi (ex.: Diagnóstico), mesmo contendo dados de estudo de mercado; não reclassifique como Briefing de Cotação só por causa disso.
